...
if (!function_exists('saveAll')) {
     ...
     * @param $field array 修改的字段
     * @param $params array 二维数组
     * @return int
     * @throws \yii\db\Exception
     */
    function saveAll($tableName, $field, $params)
    {
        //数据批量入库
...
if (!function_exists('updateeAll')) {
     ...
     * @throws \yii\db\Exception
     */
    function updateAll($tbName, $params)
    {
        $sqlArr = [];
        foreach ($params as $param) {
            $id = $param['id'];
            unset($param['id']);
            $sqlArr[] = \Yii::$app->db->createCommand()->update($tbName, $param, ['id' => $id])->getRawSql();
        }
        $sql = implode(";\n", $sqlArr) . ';';
