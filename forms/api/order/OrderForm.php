...
class OrderForm extends Model
{
    ...
    public function orderConfirm()
    {
         ...
         *处理自动佣金
         */
        $brokerageModel = New CommissionIssued();
        $brokerageModel->order_id = $this->id; // 传参才会id
        $brokerageModel->user_id = \Yii::$app->user->id;
        $brokerageModel->mall_id = \Yii::$app->mall->id;
        $brokerageModel->create_at = time();
        $brokerageModel->save();
        die;
     ...
     * @return array
     */
    public function depositCommissionIssued()
    {
       $a =  \Yii::$app->currency->setUser(User::findOne('4020'))->brokerage->select();echo "<pre>"; var_dump($a);die;
        // 获取佣金值
        // 获取佣金的20% 存入佣金发放表
    }
