...
use app\plugins\diy\models\CoreTemplateLocal;
...
class TemplateForm extends Model
{
    ...
    public function getList()
    {
        ...
        try {
            ...
            // 管理员获取所有模板
            if (\Yii::$app->role->isSuperAdmin) {
                $list = CoreTemplateLocal::find()->where(['>', 'id', 0])->orderBy(['id' => SORT_DESC])->page(
                    $res['pagination'],
                    16,
                    $this->page
                )->all();
                ...
                // 其他用户按权限获取
                $list = CoreTemplateLocal::find()->where(['in', 'id', $templateId])->orderBy(['id' => SORT_DESC])->page(
                    $res['pagination'],
                    16,
                    $this->page
                )->all();
...
//                    break;
    ...
    public function install()
    {
        ...
        try {
            $detail = CoreTemplateLocal::findOne(['id' => $this->template_id]);
            ...
            if ($coreTemplate && version_compare($coreTemplate->version, $detail['version'], '=')) {
                throw new \RuntimeException('已安装模板，无需重复安装');
            }
            ...
            if (!$coreTemplate->save()) {
                throw new \RuntimeException($this->getErrorMsg($coreTemplate));
            }
            return [
                'code' => ApiCode::CODE_SUCCESS,
                'msg' => '安装完成',
    ...
    public function uninstall()
    {
        ...
        try {
            ...
            if (!$coreTemplate) {
                throw new \Exception('找不到需要卸载的模板');
            }
            $coreTemplate->delete();
            return [
                'code' => ApiCode::CODE_SUCCESS,
